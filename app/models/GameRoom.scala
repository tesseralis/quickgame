...
class Client(messageFromJson: (String, JsValue) => Option[ServerMessage]) extends Actor {
  ...
  val iteratee = Iteratee.foreach[JsValue] { json =>
    for (kind <- (json\"kind").asOpt[String]) {
      context.parent ! messageFromJson(kind, json\"data")
    }
...
trait GameRoom[State, Mov] extends Actor {
  ...
  def messageFromJson(kind: String, data: JsValue): Option[ServerMessage] = kind match {
  ...
  override def receive = {
    case msg @ Join(name) => {
      // Constructor?
      val client = context.actorOf(Props(new Client(messageFromJson _)))
